...
Invoke-VMScript -VM $HostnameString -ScriptText "Set-DnsClientServerAddress -InterfaceAlias Ethernet0 -ServerAddresses ('DNSServer1,1DNSServer2')" -GuestCredential $vmLocalCredential -ScriptType Powershell
#Below line needs default gateway to be auto populated
Invoke-VMScript -VM $HostnameString -ScriptText "New-NetIPaddress -InterfaceAlias Ethernet0 -IPAddress $IPAddressInput -PrefixLength 24 -DefaultGateway defaultgateway" -GuestCredential $vmLocalCredential -ScriptType Powershell
...
#Configure/Initialize Disks, Requires WMF5, must be logged in as admin

$FormDisk = New-Object system.Windows.Forms.Form
$FormDisk.Text = "Disk Assignment"
$FormDisk.TopMost = $true
$FormDisk.Width = 1200
$FormDisk.Height = 1150

$DomainUser = "DPSK12\sa14"
$DomainPWord = ConvertTo-SecureString -String "Pirlo#2193" -AsPlainText -Force;
$DomainCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $DomainUser, $DomainPWord;

$DiskCIM = New-CimSession -ComputerName $HostnameString -Credential $DomainCredential
$DiskValue0 = (Get-Disk -CimSession $DiskCIM | Where-Object PartitionStyle -eq raw).Size[0]
$DiskValue1 = (Get-Disk -CimSession $DiskCIM | Where-Object PartitionStyle -eq raw).Size[1]
$DiskValue2 = (Get-Disk -CimSession $DiskCIM | Where-Object PartitionStyle -eq raw).Size[2]

$DiskCB0 = New-Object system.windows.Forms.ComboBox
$DiskCB0.Text = "Choose a drive letter"
$DiskCB0.Width = 287
$DiskCB0.Height = 20
$DiskCB0.location = new-object system.drawing.point(22, 20)
$DiskCB0.Font = "Microsoft Sans Serif,10"
$FormDisk.controls.Add($DiskCB0)

$Driveletters = [char[]]([int][char]'A'..[int][char]'Z')

Foreach ($letter in $Driveletters){
    $DiskCB0.Items.Add($letter)
}

$DriveLabel0 = New-Object system.windows.Forms.Label
$DriveLabel0.Text = "$DiskValue0"
$DriveLabel0.AutoSize = $true
$DriveLabel0.Width = 22
$DriveLabel0.Height = 10
$DriveLabel0.location = new-object system.drawing.point(50, 152)
$DriveLabel0.Font = "Microsoft Sans Serif,10"
$FormDisk.controls.Add($DriveLabel0)

<#$DiskCB1 = New-Object system.windows.Forms.ComboBox
$DiskCB1.Text = "Choose a disk"
$DiskCB1.Width = 287
$DiskCB2.Height = 20
$DiskCB1.location = new-object system.drawing.point(22, 20)
$DiskCB1.Font = "Microsoft Sans Serif,10"
$FormDisk.controls.Add($DiskValue1)

$DiskCB2 = New-Object system.windows.Forms.ComboBox
$DiskCB2.Text = "Choose a disk"
$DiskCB2.Width = 287
$DiskCB2.Height = 20
$DiskCB2.location = new-object system.drawing.point(22, 20)
$DiskCB2.Font = "Microsoft Sans Serif,10"
$#>

#button DC
$AssignDisksButton = New-Object system.windows.Forms.Button
$AssignDisksButton.Text = "Select"
$AssignDisksButton.Width = 100
$AssignDisksButton.Height = 40
$AssignDisksButton.location = new-object system.drawing.point(22, 50)
$AssignDisksButton.Font = "Microsoft Sans Serif,10"
$FormDisk.controls.Add($CreateDC)
$AssignDisksButton.Add_Click( {$FormDisk.Close()})

#Final DC Form related tasks
[void]$FormDisk.ShowDialog()
$DiskValue0 = $DiskCB0.SelectedItem
$DiskValue1 = $DiskCB1.SelectedItem
$DiskValue2 = $DiskCB2.SelectedItem
$FormDisk.Dispose()

#Get-Disk -CimSession $HostnameString | Sort-Object Number | Where-Object PartitionStyle -eq 'raw' | Initialize-Disk -PartitionStyle MBR
#New-Partition -CimSession $HostnameString -DiskNumber 1 -UseMaximumSize -DriveLetter G
#Format-Volume -CimSession $HostnameString -DriveLetter G -NewFileSystemLabel Data -Confirm:$false

#New-Partition -CimSession "$HostnameString.dpsk12.org" -DiskNumber 2 -UseMaximumSize -DriveLetter N
#Format-Volume -CimSession "$HostnameString.dpsk12.org" -DriveLetter N -NewFileSystemLabel Logs -Confirm:$false

#New-Partition -CimSession $HostnameString -DiskNumber 2 -UseMaximumSize -DriveLetter E
#Format-Volume -CimSession $HostnameString -DriveLetter E -NewFileSystemLabel Apps -Confirm:$false
